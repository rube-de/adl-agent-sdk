...
def test_logger_creates_directory(log_dir: Path):
    logger = IssueLogger(log_dir, "owner-repo", 42)
    logger.log_event("test", {"key": "value"})
    assert (log_dir / "owner-repo-42").is_dir()
...
def test_log_event_writes_jsonl(log_dir: Path):
    ...
    log_file = log_dir / "owner-repo-42" / "log.jsonl"
...
def test_update_context(log_dir: Path):
    ...
    ctx_file = log_dir / "owner-repo-42" / "context.jsonl"
...
def test_write_state(log_dir: Path):
    ...
    state_file = log_dir / "owner-repo-42" / "state.json"
